...
if (!process.env.MONGODB_URI) {
  ...
  console.error(
    "Please create a .env file in the root directory with MONGODB_URI defined.",
  );
...
mongoose
  ...
  .catch((err) => {
    ...
    if (
      ...
      console.error(
        "   Your current IP address is not on the MongoDB Atlas IP Access List.",
      );
      console.error("\n   To fix this:");
      console.error("   1. Log into MongoDB Atlas: https://cloud.mongodb.com/");
      console.error(
        "   2. Go to your project -> Network Access (or Security -> Network Access)",
      );
      console.error('   3. Click "Add IP Address" or "Add Current IP Address"');
      console.error(
        "   4. Or add 0.0.0.0/0 to allow access from anywhere (less secure, only for development)",
      );
