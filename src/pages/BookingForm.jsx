...
const BookingForm = () => {
  ...
  const loadTutorAndModules = async () => {
    try {
      ...
      const tutorSubjects = tutorData?.tutorProfile?.subjects || [];
      const modulesList = tutorSubjects.map(s => ({
        value: s.moduleCode,
        label: `${s.moduleCode} - ${s.name}`
      }));

      setSubjects(modulesList);

      if (modulesList.length > 0) {
        setFormData(prev => ({ ...prev, subject: modulesList[0].label }));
      }
    } catch (error) {
              ...
              <div className="bg-white rounded-lg shadow p-6 mb-6">
                <div className="flex items-center gap-4 pb-6 border-b border-gray-200">
                  ...
                  <div>
                    ...
                    <p className="text-gray-600">{tutor.year} | {tutor.course}</p>
                  </div>
                </div>
              </div>
